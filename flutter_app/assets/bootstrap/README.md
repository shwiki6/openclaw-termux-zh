...
The standard APK bundles the Ubuntu base and Node.js arm64 archives listed
in `pubspec.yaml` so first setup can avoid those network downloads. Additional
archives in this folder are for local caching, testing, or building a special
offline package. The setup wizard can also accept a local prebuilt rootfs
archive or a direct prebuilt rootfs download URL.
